...
#ifndef KOCHAVAENHANCEMENT_SWIFT_H
#define KOCHAVAENHANCEMENT_SWIFT_H
...
# undef any
# pragma clang attribute push(__attribute__((external_source_symbol(language="Swift", defined_in="KochavaEnhancement",generated_declaration))), apply_to=any(function,enum,objc_interface,objc_category,objc_protocol))
...
@class NSString;
@class KVAContext;
@class KVAEnhancement_General;
@class KVANetworking;

/// The class Enhancement provides an interface between a host application and Kochava’s Enhancement servers.
SWIFT_CLASS_NAMED("Enhancement")
@interface KVAEnhancement : NSObject
/// A shared instance, for convenience.
/// This is the preferred way of using a enhancement instance.  To complete the integration you must call func <code>Enhancement/start()</code>, or else start a controlling linked module.  You may alternatively use a constructor to create your own enhancement.  The shared instance simplifies your implementation as you do not need to store a enhancement instance somewhere in a public location in your own code, and you do not need to manually link together the clients of any optional products.
/// By default this instance will use the default storage location equivalent to calling <code>Enhancement/init(storageIdentifier:)</code> with storageIdentifier nil.  If you wish to specify an alternative storage location, see var <code>sharedStorageIdentifier</code>.
SWIFT_CLASS_PROPERTY(@property (nonatomic, class, readonly, strong) KVAEnhancement * _Nonnull shared;)
+ (KVAEnhancement * _Nonnull)shared SWIFT_WARN_UNUSED_RESULT;
...
SWIFT_CLASS_PROPERTY(@property (nonatomic, class, readonly, strong) KVAEnhancement * _Nullable shared_optional;)
+ (KVAEnhancement * _Nullable)shared_optional SWIFT_WARN_UNUSED_RESULT;
...
/// Create a enhancement instance.  You rarely create instances of class Enhancement.  Instead you typically configure the provided shared instance and then start it using the start method.  See static var <code>shared</code>.
/// <h2>Example</h2>
/// \code
/// class MyClass
/// {
...
/// <h2>Example</h2>
/// \code
/// // enhancementConfigureObject
/// let enhancementConfigureObject =
/// [
///     "$override.append$":
///     [
///         "privacy":
...
///
/// // Enhancement
/// Enhancement.shared.configure(with: enhancementConfigureObject, context: .host)
/// Enhancement.shared.general.registerIdentity(
///     nameString: "_SOME_NAME_",
///     valueString: "_SOME_VALUE_"
/// )
/// Enhancement.shared.start()
///
...
/// Configure the instance with an object.
/// See func <code>Enhancement/configure(with:context:)</code>.
...
/// Start the enhancement client.
- (void)start;
/// Start the enhancement client with advanced options.
/// This method can be used to start an instance of a enhancement which was created from decoded JSON.
/// important:
...
/// Stop is similar to <code>Enhancement/invalidate()</code> (or the use of KochavaEnhancement.Product/reset(deleteLocalDataBool:) or KochavaEnhancement.Product.shutdown(deleteLocalDataBool:) which wrap <code>Enhancement/invalidate()</code>) in that both will block new execution, though through different mechanisms;  however, it is different in that stop maintains state and can be started again.  In contrast <code>Enhancement/invalidate()</code> discards state, cannot be re-started, and in the case of the shared instance is automatically released.
...
- (void)stop;
/// Stop the enhancement client with advanced options.
/// The primary action of this method is to reset the Networking.General.startedTask, which will cause any task which is dependent upon that task to block the initiation of new execution.  The current state of the system is fundamentally maintained;  however, essentially the same things which would not execute prior to having been started will not execute while stopped.
/// Stop is similar to <code>Enhancement/invalidate()</code> (or the use of KochavaEnhancement.Product/reset(deleteLocalDataBool:) or KochavaEnhancement.Product.shutdown(deleteLocalDataBool:) which wrap <code>Enhancement/invalidate()</code>) in that both will block new execution, though through different mechanisms;  however, it is different in that stop maintains state and can be started again.  In contrast <code>Enhancement/invalidate()</code> discards state, cannot be re-started, and in the case of the shared instance is automatically released.
/// Stop is similar to class Networking var sleepBool in that both will block new execution through a similar mechanism;  however, it is different in that stop essentially covers all aspects of exectution which are at an inverse of start.  In contrast, class Networking var sleepBool exempts certain key features and these can continue to execute while sleep is in effect.
/// \param logMessagePrintBool A boolean indicating if a log messages consistent with this being a call to a public entry point should be printed.
...
/// Invalidate the enhancement client.
...
- (void)invalidate;
/// Invalidate the enhancement client with advanced options.
/// See func <code>Enhancement/invalidate()</code>
/// \param logMessagePrintBool A boolean indicating if a log messages consistent with this being a call to a public entry point should be printed.
///
/// \param asyncBool A boolean indicating if the work within this method should asynchronously dispatch to the globalSerial queue.
///
- (void)invalidateWithLogMessagePrintBool:(BOOL)logMessagePrintBool asyncBool:(BOOL)asyncBool;
/// A feature which encapsulates all of the general aspects of an enhancement instance not belonging to any other encapsulated features.
/// <h1>See</h1>
/// Class <code>Enhancement/General</code>.
@property (nonatomic, readonly, strong) KVAEnhancement_General * _Nonnull general;
...
@interface KVAEnhancement (SWIFT_EXTENSION(KochavaEnhancement)) <JSExport>
@end


@interface KVAEnhancement (SWIFT_EXTENSION(KochavaEnhancement))
@end


/// A feature which encapsulates all of the general aspects of a enhancement instance not belonging to any other feature components.
SWIFT_CLASS_NAMED("General")
@interface KVAEnhancement_General : NSObject
- (nonnull instancetype)init SWIFT_UNAVAILABLE;
+ (nonnull instancetype)new SWIFT_UNAVAILABLE_MSG("-init is unavailable");
@end
...
/// A class which defines the enhancement product.
...
@interface KochavaEnhancement_Product : NSObject
